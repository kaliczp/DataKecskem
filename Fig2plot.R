...
yeartext <- c(tick.year + c(183, 183, 183, 160, 70))
text(x = yeartext, y = rep(-9.3, length(tick.year)), labels = format(yeartext, format = "%Y"), cex = 2, col = "lightgray")
axis(2, at = c(-8, -8.5, -9))
mtext("GW forest [m below surface]", side = 2, line = 3, at = -8.5, las = 0)
grid(nx = NA, ny = NULL)
...
mtext("GW grassland [m below surface]", side = 4, line = 3, at = -4, las = 0, col = 2)
...
legend(as.Date("2017-03-25"), 165, legend = c("Forest", "Open-field", "Precipitation"), col = c(1:2, "blue"), lty=1, lwd = c(2, 2, 3) , bg = "white")
